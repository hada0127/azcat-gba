...
void test_game_constants(void) {
    ...
    TEST_ASSERT_EQUAL_INT32(FP(128), CAT_LAND_Y);
    TEST_ASSERT_EQUAL_INT32(FP_FROM_FLOAT_CONST(0.067f), PLAYER_ACCEL_INC);
    TEST_ASSERT_EQUAL_INT32(FP_FROM_FLOAT_CONST(0.167f), CAT_GRAVITY);
    TEST_ASSERT_EQUAL_INT32(60, BOMB_DURATION);
    TEST_ASSERT_EQUAL_INT32(3, FRAME_RATE_RATIO);
}

void test_fp_player_speed(void) {
    /* 1.667 + 0.067(누적) + player_accel/3 시뮬레이션 */
    s32 speed = PLAYER_BASE_SPEED + PLAYER_ACCEL_INC + FP(1) / FRAME_RATE_RATIO;
    s32 expected = FP_FROM_FLOAT_CONST(1.667f) + FP_FROM_FLOAT_CONST(0.067f) + FP(1) / 3;
